...
@section('content')
  <div class="col-sm-10 text-left">
    ...
    <div class="text-right student">
      <a href="{{ asset('/kythi/export/'.$kythi->id) }}" class="btn btn-primary default">export</a>
    </div>
            ...
            <tr class="bailam_hien">
              <td>
                <table class="table table-striped text-center">
                  ...
                  <tbody>
                  @foreach($kythis as $key => $k)
                    <tr>
                      <td>{{ $key+1 }}</td>
                      <td>{{ $k->user_id }}</td>
                      <td>
                        <a href="{{ asset('/kythi/detail-user/'.$k->user_id) }}">
                          {{ $k->users->username }}
                        </a>
                      </td>
                      <td>{{ $k->users->name }}</td>
                      <td>{{ $k->users->birthday }}</td>
                      <td>{{ $arr_de[$k->user_id] }}</td>
                      <td>{{ $k->trangthai }}</td>
                      <td>{{ $arr_diemcauhoi[$k->user_id] }}</td>
                      <td>{{ $arr_diemthisinh[$k->user_id] }}</td>
                      @if($k->trangthai == Config('constants.status.2'))
                        <td>
